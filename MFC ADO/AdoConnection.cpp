...
long vartol(const _variant_t& var)
{
	...
	case VT_LPSTR://字符串
	case VT_LPWSTR://字符串
		value = atol((char *)(LPCSTR)(_bstr_t)var);
		break;
	case VT_NULL:
	case VT_EMPTY:
		value = 0;
...
double vartof(const _variant_t& var)
{
	...
	case VT_LPSTR://字符串
	case VT_LPWSTR://字符串
		value = atof((char *)(LPCSTR)(_bstr_t)var);
		break;
	case VT_NULL:
	case VT_EMPTY:
		value = 0;
...
BOOL CAdoConnection::Open(LPCTSTR lpszConnect, long lOptions)
{
	...
	if (strcmp((char *)(LPCSTR)lpszConnect, (char *)(LPCSTR)_T("")) != 0)
	{
...
{
	//Oracle 的连接：
	CString strConnection = _T("Provider=MSDAORA;Data Source=") + server +
