...
BEGIN_MESSAGE_MAP(CGridCtrlDemoDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON_UNMERGE, OnButtonUnmerge)
...
//                 SWP_SHOWWINDOW);
	...
	//m_Grid.MergeCells(0,0,0,1);
	//m_Grid.MergeCells(0,2,0,4);
	//m_Grid.MergeCells(1,2,2,3);
	//m_Grid.MergeCells(3,0,5,0);
    UpdateMenuUI();
...
void CGridCtrlDemoDlg::OnCellCombo() 
{
    ...
    if (!m_Grid.SetCellType(1,1, RUNTIME_CLASS(CGridCellCheckCombo)))
        return;
    ...
    CGridCellCheckCombo *pCell = (CGridCellCheckCombo*) m_Grid.GetCell(1,1);
    pCell->SetOptions(options);
    pCell->SetStyle(CBS_DROPDOWN); //CBS_DROPDOWN, CBS_DROPDOWNLIST, CBS_SIMPLE

   if (!m_Grid.SetCellType(1,2, RUNTIME_CLASS(CGridCellCheckCombo)))
        return;

    m_Grid.SetItemText(1,2, _T("Option 4"));

    CStringArray options1;
    options1.Add(_T("Option 4"));
    options1.Add(_T("Option 5"));
    options1.Add(_T("Option 6"));

    pCell = (CGridCellCheckCombo*) m_Grid.GetCell(1,2);
    pCell->SetOptions(options1);
    pCell->SetStyle(CBS_DROPDOWN); //CBS_DROPDOWN, CBS_DROPDOWNLIST, CBS_SIMPLE
...
void CGridCtrlDemoDlg::OnButtonUnmerge() 
{
	
	m_Grid.UnMergeSelectedCells();
	
}

void CGridCtrlDemoDlg::OnButtonMerge() 
{
